...
void main () {
  ...
  vec4 startPosData = texture2D(startPos, uv);
  ...
  float speedTime = 35.0 * time;
  vec3 mouseMini = mouse * 1.0;
  mouseMini.x = ((sin(speedTime) * sin(speedTime)) - 0.5)  * 1.0;
  mouseMini.y = sin(speedTime) * cos(speedTime) * 1.0;

  vec3 randomBall = ballify(vec3(x, y, z) + mouseMini, 1.0);

  vec3 pt1 = ballify(randomBall + nextPos, 5.0);
  pt1.x += cos(pt1.x * 3.0 + time) * sin(pt1.x * 3.0 + time);
  pt1.y += cos(pt1.y * 3.0 + time) * sin(pt1.y * 3.0 + time);
  pt1.z += cos(pt1.z * 3.0 + time) * sin(pt1.z * 3.0 + time);

  vec3 pt2 = ballify(randomBall + nextPos, 15.0);
  pt2.z += rand(pt2.xy + speedTime);

  nextPos = mix(pt1, pt2, smoothstep(0.0, 2.0, abs(distance(mouseMini, vec3(0.0))) / 1.0));

  vec3 logo = startPosData.rgb * 15.0;

  float inter = abs(distance(mouseMini, vec3(0.0))) / 0.5;
  float flip = smoothstep(0.0, 1.0, abs(sin(time * 2.0)));
  nextPos += getDiff(nextPos, mouse * 17.0) * 50.0;
  nextPos.rgb = mix(logo, nextPos, 1.0 - flip);
