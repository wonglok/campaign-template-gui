...
export const makeCubeTexture = (api) => {
  ...
  class TouchTexture {
    constructor() {
      ...
      this.colorDot = new THREE.Color('#FCACBB')
      this.colorBG = new THREE.Color('#FCACBB')
      this.hueSpread = 0 / 10
    ...
    drawPoint(point) {
      ...
      color = `${(this.hueDot - variant / 2 + intensity * variant * point.vx).toFixed(0)}, ${this.satuationDot}%, ${
        this.lightnessDot
      }%`
  ...
  let cubeTexture = new THREE.CubeTexture([t.canvas, t.canvas, t.canvas, t.canvas, t.canvas, t.canvas])
  let touchTextures = [t]
