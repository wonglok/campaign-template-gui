      ...
      {pts.map((pt, i) => {
        return (
          <RayGrab
            key={'ctrl__' + i}
            onMove={(ev) => {
              ev.intersection.object.getWorldPosition(v3)
...
function ParticleRelayCore({ surfaceMesh }) {
  ...
  let cursorA = useMemo(() => {
    let o3 = new Mesh(
      ...
      new MeshPhysicalMaterial({ transmission: 1, thickness: 5, roughness: 0, ior: 1.5 }),
    )
    o3.position.x = -3
    return o3
  }, [])
    ...
    <>
      <RayGrab>
        <Sphere
          ...
          scale={[1 * s, 1.5 * s, 1 * s]}
        >
          <meshStandardMaterial metalness={1} color={'#00ff00'}></meshStandardMaterial>
        </Sphere>
      </RayGrab>
