...
export function ParticleRelay() {
  ...
  let applyEmissionGeometryChange = useCallback(() => {
    ...
    tt.current = setTimeout(() => {
      ...
      // let inner = scene.getObjectByName('inner')
        ...
        <Geometry ref={csgRef}>
          ...
          >
            <group position={[0, -7, 0]}>
              <Addition>
                <boxGeometry args={[75, 1, 75]}></boxGeometry>
              </Addition>
            </group>
          </PivotControls> */}
          ...
          <PivotControls
            ...
            anchor={[0, 0, 0]}
          >
            <group position={[-15 - 10, -5.5, -0.7656101630036858]}>
              <Addition>
                <boxGeometry args={[1, 24, 24]}></boxGeometry>
              </Addition>
            </group>
          </PivotControls>
...
function CurveYo() {
  let [pts, setPts] = useState(() => {
    return [
      {
        position: [-32.59986171798357, -5.49996803833001, 11.34354193816161],
      },
      {
        position: [-27.518861717988102, -5.499968038329936, -11.02145806183852],
      },
      {
        position: [-1.0894381072096269, -5.49996803832987, -18.772270442347086],
      },
      {
        position: [18.199944974907318, -5.499968038329813, -16.430832687718585],
      },
      {
        position: [23.782944974906066, -5.499968038300535, -1.2558326877186943],
      },
      {
        position: [20.68580311089355, -4.542147428583418, 15.688996964054951],
      },
    ]
  }, [])
...
function ParticleRelayCore({ surfaceMesh }) {
  ...
  let unitGeomtry = new PlaneGeometry(0.5, 2)
  ...
  return (
    <>
      {/* <directionalLight position={[0, -50, -30]} intensity={5}></directionalLight> */}
...
export function CoreEngine({
  ...
  useFrame(() => {
    ...
    let avatarAction = useFinger.getState().avatarAction

    if (avatarAction === 'idle') {
      colorRef.current = '#ffffff'
    }
    if (avatarAction === 'attract') {
      colorRef.current = '#ff00ff'
    }
    if (avatarAction === 'push') {
      colorRef.current = '#00ffff'
    }
  })
