...
export class LokLokWiggleSimulation {
  constructor({
    ...
    this.getAltHeadList = getAltHeadList;
    this.getRebornPosition = getRebornPosition;
    this.wait = this.setup({ core });
  }
  async setup({ core }) {
    let renderer = this.renderer;
    let gpu = new CustomGPU(this.WIDTH, this.HEIGHT, renderer);
    ...
    this.positionUniforms["headList2"] = {
      value: this.getAltHeadList(),
    };

    this.positionUniforms["rebornPosition"] = {
      value: this.getRebornPosition(),
    };

    //
    this.positionUniforms["jawOpen"] = { value: 0 };
    this.positionUniforms["browInnerUp"] = { value: 0 };

    this.core.onLoop(() => {
      this.positionUniforms["jawOpen"] = { value: this.params.jawOpen };
      this.positionUniforms["browInnerUp"] = { value: this.params.browInnerUp };
      this.positionUniforms["headList"] = {
        value: this.getHeadList(),
      };
      this.positionUniforms["headList2"] = {
  ...
  positionShader() {
    ...
    return /* glsl */ `
      ...
      #include <common>
        ...
        vec2 myuvv = vec2(0.0, currentIDX / ${this.HEIGHT.toFixed(1)});
        vec4 rebornPos = texture2D(rebornPosition, myuvv);
        vec4 headpos2 = texture2D(headList, myuvv);
        ...
        if (floor(currentIDX) == 0.0) {
          ...
          positionChain.xyz *= 1.0 + (browInnerUp) / 75.0;// + 0.0135;

          gl_FragColor = vec4(positionChain, 1.0);
        }

        if (length(rebornPos.xyx - headpos2.rgb) <= 0.1) {
...
export class LokLokWiggleDisplay {
  ...
  async setup({ core }) {
    ...
    // let camera = await core.ready.camera;
    // let renderer = await core.ready.gl;
...
export class NoodleO3 {
  ...
  async setup({ core }) {
    ...
    let sim = new LokLokWiggleSimulation({
      ...
      getAltHeadList: this.getAltHeadList,
      getRebornPosition: this.getRebornPosition,
      core,
    ...
    let display = new LokLokWiggleDisplay({ core, sim, params: this.params });
